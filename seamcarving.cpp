...
int main() {
  ...
  if(targetHeight <= 0){
    cout << "Error: targetHeight must be greater than 0. You entered" << targetHeight << endl;
    return 1;
  }
  
  if(targetHeight >= height){
    cout << "Error: targetHeight must be less than height, " << targetHeight << " is greater than " << height << endl;
    return 1;
  }

  if(targetWidth <= 0){
    cout << "Error: targetHeight must be greater than 0. You entered" << targetWidth << endl;
    return 1;
  }
  
  if(targetWidth >= width){
    cout << "Error: targetWidth must be less than width, " << targetWidth << " is greater than " << width << endl;
    return 1;
  }
